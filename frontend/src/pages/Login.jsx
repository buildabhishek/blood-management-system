...
export default function Login({ onLogin }) {
  ...
  return (
    <div className="min-h-screen flex" style={{ backgroundColor: RC.pinkSoft }}>
      ...
      {/* ── Left brand panel ────────────────────────────────────────────────── */}
      <div className="hidden md:flex flex-col items-center justify-center w-2/5 px-10 relative"
        style={{ backgroundColor: RC.crimson }}>
        <div className="absolute top-0 left-0 w-full h-2" style={{ backgroundColor: RC.green }} />
        ...
        <div className="absolute bottom-0 left-0 w-full h-2" style={{ backgroundColor: RC.green }} />
      </div>
              ...
              <div className="grid grid-cols-3 gap-2">
                {ROLES.map(r => (
                  <button key={r.value} type="button"
                    onClick={() => { setForm(f => ({ ...f, role: r.value })); setFieldErrors({}); }}
          ...
          <form onSubmit={handleSubmit} noValidate>
            <div className="space-y-3">
              ...
              {mode === "register" && (
                <Field label="Full Name" error={fieldErrors.name}>
                  <RCInput value={form.name} onChange={setField("name")}
                    placeholder="Your full name" hasError={!!fieldErrors.name} />
                </Field>
              )}

              <Field label="Phone Number" error={fieldErrors.phone}>
                <RCInput value={form.phone} onChange={setField("phone")}
                  placeholder="10-digit mobile number" inputMode="numeric"
                  hasError={!!fieldErrors.phone} />
              </Field>

              <Field label="Password" error={fieldErrors.password}>
                <RCInput type="password" value={form.password} onChange={setField("password")}
                  placeholder="Minimum 6 characters" hasError={!!fieldErrors.password} />
              </Field>
              ...
              {mode === "register" && (form.role === "HOSPITAL" || form.role === "BLOOD_BANK") && (
                <>
                  <Field
                    ...
                    <RCInput value={form.entityName} onChange={setField("entityName")}
                      placeholder="Official registered name" hasError={!!fieldErrors.entityName} />
                  </Field>

                  <Field label="Address (optional)">
                    <RCInput value={form.address} onChange={setField("address")}
                      placeholder="Full address" />
                  </Field>

                  <div className="grid grid-cols-2 gap-2">
                    <Field label="Latitude (optional)">
                      <RCInput type="number" value={form.latitude} onChange={setField("latitude")}
                        placeholder="e.g. 19.0760" />
                    </Field>
                    <Field label="Longitude (optional)">
                      <RCInput type="number" value={form.longitude} onChange={setField("longitude")}
            ...
            {success && (
              <div className="mt-4 p-3 rounded-xl text-sm font-medium"
                style={{ backgroundColor: RC.greenLight, border: `1.5px solid ${RC.greenMid}`, color: RC.greenDark }}>
                {success}
              </div>
            )}
            ...
            {globalError && (
              <div className="mt-4 p-3 rounded-xl text-sm font-medium"
                style={{ backgroundColor: "#FDE8F0", border: `1.5px solid ${RC.crimson}`, color: RC.crimson }}>
                <p className="font-bold mb-1">⚠️ {mode === "login" ? "Sign in failed" : "Registration failed"}</p>
